...
def main(config):
    ...
    agent_service = config.get('agent_service')
    for agent_file in agent_files:
        for i in range(num):
            commands = 'python {} {} {} {}'.format(agent_file, port_, agent_name, "'"+agent_service+"'")
            commands = shlex.split(commands)
            p = subprocess.Popen(commands, stdin=subprocess.PIPE)
            processes.append(p)
            time.sleep(0.5)
...
@click.option('--agent_service', default='service')
@click.option('--agent_name', default='name')
def start_runtime(num, agent_files, port, pade_ams, pade_web, pade_sniffer, username, password, agent_name, agent_service):
    config = dict()
    ...
    config['agent_service'] = agent_service
